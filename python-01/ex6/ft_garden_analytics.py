...
class GardenManager:
    ...
    def get_report(self) -> None:
        ...
        n_regular = sum(1 for p in self.plants if type(p) is Plant)
        n_flowering = sum(1 for p in self.plants if type(p) is FloweringPlant)
        n_prize = sum(1 for p in self.plants if type(p) is PrizeFlower)

        print(
                f"Plant types: {n_regular} regular, {n_flowering} flowering, "
                f"{n_prize} prize flowers"
            )
