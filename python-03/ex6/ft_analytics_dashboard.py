...
def dict_comprehensions_demo(
    ...
    achievement_counts = {
            player: len(achvs) for player, achvs in achievements.items()
        }
...
def set_comprehensions_demo(
    ...
    unique_achievements = {
            achv for achvs in achievements.values() for achv in achvs
        }
...
def main() -> None:
    ...
    total_unique_achvs = len(
            {a for achvs in player_achievements.values() for a in achvs}
        )
    ...
    print(
        ...
        f"({top_score} points, {top_achvs} achievements)"
    )
